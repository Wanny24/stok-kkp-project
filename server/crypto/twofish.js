...
function encryptBlock(plain, io, cipher, oo, session) {
  const [sBox, sKey] = session;
...
function decryptBlock(cipher, io, plain, oo, session) {
  const [sBox, sKey] = session;
...
class TwoFishCrypto {
    constructor(key) {
        ...
        const keyUint8 = new Uint8Array(this.key);
        this.session = makeSession(keyUint8);
        this.identifier = 'TWOFISH';
    }
    ...
    encrypt(text) {
        ...
        let prevBlock = iv;
        const tempIn = new Uint8Array(16);
        const tempOut = new Uint8Array(16);
        
        for (let i = 0; i < paddedPlaintext.length; i += 16) {
            for (let j = 0; j < 16; j++) {
                tempIn[j] = paddedPlaintext[i + j] ^ prevBlock[j];
            }
            
            encryptBlock(tempIn, 0, tempOut, 0, this.session);
            
            for (let j = 0; j < 16; j++) {
                ciphertext[i + j] = tempOut[j];
            }
            
            prevBlock = ciphertext.slice(i, i + 16);
        }
        
        return this.identifier + ':' + iv.toString('hex') + ':' + ciphertext.toString('hex');
    }

    decrypt(encryptedText) {
        try {
            const [identifier, ivHex, ciphertextHex] = encryptedText.split(':');
            ...
            for (let i = 0; i < ciphertext.length; i += 16) {
                for (let j = 0; j < 16; j++) {
                    tempIn[j] = ciphertext[i + j];
                }
                
                decryptBlock(tempIn, 0, tempOut, 0, this.session);
                
                for (let j = 0; j < 16; j++) {
                    plaintext[i + j] = tempOut[j] ^ prevBlock[j];
                }
                
                prevBlock = tempIn.slice(0);
            }
