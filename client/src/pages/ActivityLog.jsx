    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                <button onClick={() => navigate(-1)} className="mb-6 inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-5 py-2 rounded-full hover:bg-white transition-all">
                    ...
                    <div className="p-6 bg-gray-50 border-b border-gray-100">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select value={filterUsername} onChange={(e) => setFilterUsername(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                                    <option value="">Semua User</option>
                                    {usernames.map(username => (<option key={username} value={username}>{username}</option>))}
                                </select>
                            </div>
                            <div className="flex items-end">
                                <button onClick={resetFilters} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300 transition-all">
                                    <i className="fas fa-undo-alt mr-1"></i> Reset
                                </button>
                            </div>
                        </div>
                        <div className="mt-3 text-xs text-gray-500">
                            <i className="fas fa-chart-line mr-1"></i> Menampilkan {filteredLogs.length} dari {logs.length} log
                        </div>
                    </div>
                            ...
                            filteredLogs.map((log) => (
                                <div key={log.id} className="border-b border-gray-100 pb-3">
                                    <div className="flex items-center gap-2 text-sm text-gray-600 mb-1">
                                        ...
                                        <span className="text-xs">{new Date(log.timestamp).toLocaleString('id-ID', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                        {!log.decrypted && (<span className="text-xs text-amber-500 ml-2"><i className="fas fa-lock"></i> Terenkripsi</span>)}
                                    </div>
